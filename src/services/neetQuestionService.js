...
export const saveNeetQuestions = async (subject, questions, teacherUid) => {
    try {
        const response = await fetch(`/api/v1/neet/${subject}`, {
            method: 'POST',
...
export const getNeetQuestions = async (subject, topic, subTopic) => {
    try {
        ...
        const response = await fetch(`/api/v1/neet/${subject}?${params.toString()}`);
...
export const deleteNeetQuestion = async (subject, questionId) => {
    try {
        const response = await fetch(`/api/v1/neet/${subject}/${questionId}`, {
            method: 'DELETE'
        });
        return response.ok;
    } catch (error) {
...
export const deleteNeetTopic = async (subject, topic, subTopic) => {
    try {
        ...
        const response = await fetch(`/api/v1/neet/${subject}/topic?${params.toString()}`, {
            method: 'DELETE'
        });
        return response.ok;
    } catch (error) {
...
export const renameNeetTopic = async (subject, oldTopic, newTopic, oldSubTopic, newSubTopic) => {
    try {
        const response = await fetch(`/api/v1/neet/${subject}/topic`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                old_topic: oldTopic,
...
export const getNeetTopics = async (subject) => {
    try {
        const response = await fetch(`/api/v1/neet/${subject}/topics`);
        if (!response.ok) return [];
        return await response.json();
    } catch (error) {
...
export const uploadNeetQuestionsFile = async (subject, file, topic, subTopic, questionType, teacherUid) => {
    try {
        ...
        const response = await fetch(`/api/v1/neet/${subject}/upload`, {
            method: 'POST',
            body: formData
        });
...
export const generateNeetAssessment = async (subject, topics, subTopics = [], totalQuestions = 0, distribution = null) => {
    try {
        ...
        const response = await fetch(`/api/v1/neet/assessment/generate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
        });
...
export const saveNeetAssessment = async (payload, teacherUid) => {
    try {
        ...
        const response = await fetch('/api/v1/neet/assessment/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export const getNeetAssessments = async (subject, teacherUid) => {
    try {
        let url = `/api/v1/neet/${subject}/assessments`;
        if (teacherUid) {
...
export const getNeetAssessmentById = async (id) => {
    try {
        const response = await fetch(`/api/v1/neet/assessment/${id}`);
