...
export const getAssignedGrades = async (teacherUid) => {
    try {
        const response = await fetch(`/api/v1/dashboard/teacher/stats?uid=${teacherUid}`);
        if (response.ok) {
...
export const getStudentsByGrade = async (teacherUid, grade) => {
    try {
        const response = await fetch(`/api/v1/dashboard/teacher/students?uid=${teacherUid}&grade=${encodeURIComponent(grade)}`);
        if (response.ok) {
...
export const getStudentDashboardData = async (teacherUid, studentUid, childId) => {
    try {
        ...
        const response = await fetch(
            `/api/v1/dashboard/teacher/student-detail?uid=${teacherUid}&studentUid=${studentUid}&childId=${childId}`
        );
