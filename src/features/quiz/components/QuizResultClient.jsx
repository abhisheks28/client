...
const QuizResultClient = () => {
        ...
        const loadReport = async () => {
            try {
                ...
                if (phoneParam && adminView === 'true') {
                    ...
                    const response = await fetch(`/api/v1/reports/?uid=${encodeURIComponent(normalizedPhone)}&childId=${childIdParam || ''}`);
                    if (response.ok) {
                ...
                // Case 1: Teacher/Specific Report View
                    ...
                    const response = await fetch(`/api/v1/reports/?uid=${encodeURIComponent(studentUidParam)}&childId=${childIdParam || ''}`);
                    if (response.ok) {
                        const res = await response.json();
                        if (res.success && res.data && res.data.length > 0) {
                            const reports = res.data.map(r => ({ ...JSON.parse(r.report_json), reportId: r.report_id, timestamp: r.created_at }));
                ...
                // Case 3: Standard User History/Latest Report
                    ...
                    // Else fetch latest for user
                        ...
                        const response = await fetch(`/api/v1/reports/?uid=${encodeURIComponent(user.uid)}`); // Use auth UID
                        if (response.ok) {
                            const res = await response.json();
                            if (res.success && res.data && res.data.length > 0) {
                                const reports = res.data.map(r => ({ ...JSON.parse(r.report_json), reportId: r.report_id, timestamp: r.created_at }));
                ...
                if (targetReport) {
                    ...
                    setReportState({
                        ...
                        type: targetReport.type || targetReport.category || 'ASSESSMENT',
                        grade: targetReport.grade || "", // If available in report json
                    });
                }
            } catch (error) {
    ...
    useEffect(() => {
        const fetchSkillProgress = async () => {
            ...
            try {
                const response = await fetch(`/api/v1/skill-practice/progress/${reportId}?uid=${encodeURIComponent(user.uid)}`);
                if (response.ok) {
    ...
    // Check if this is a NEET assessment to hide the Learning Plan
    // Enhanced NEET detection
    const isNeet = React.useMemo(() => {
        const typeCheck = (quizSession?.userDetails?.testType === 'NEET') ||
            ((displayGrade || "").toString().toUpperCase().includes("NEET")) ||
            (reportState?.type === 'NEET') ||
            (reportState?.category === 'NEET');

        if (typeCheck) return true;

        // Heuristic: Check for NEET subjects in topics
        const topics = Object.keys(reportState?.topicFeedback || {});
        const hasNeetSubjects = topics.some(t => ['Physics', 'Chemistry', 'Biology'].some(sub => t.includes(sub)));

        console.log("🔍 [Debug isNeet]", {
            typeCheck,
            hasNeetSubjects,
            displayGrade,
            reportType: reportState?.type,
            testType: quizSession?.userDetails?.testType,
            topics
        });

        return hasNeetSubjects;
    }, [quizSession, reportState, displayGrade]);
