...
const DashboardClient = () => {
        ...
        const fetchReports = async () => {
            if (activeChildId) {
                ...
                try {
                    ...
                    const response = await fetch(`/api/v1/reports?uid=${currentUid}&childId=${activeChildId}`);
    ...
    const handleSaveChild = async () => {
        ...
        try {
            const response = await fetch(`/api/v1/users/${uid}/child`, {
                method: 'POST',
    ...
    const handleUpdateChild = async () => {
        ...
        try {
            ...
            const endpoint = isStudent
                ? `/api/v1/users/me`
                : `/api/v1/users/${uid}/child/${editingChildId}`;

            const method = isStudent ? 'PATCH' : 'PUT';

            const payload = isStudent
                ? { display_name: name, grade }
                : { name, grade };
            ...
            if (response.ok && (result.user_id || result.success)) {
