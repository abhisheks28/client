...
export const getDaysProgress = async (reportId, uid) => {
    try {
        const response = await fetch(`/api/v1/skill-practice/progress/${reportId}?uid=${encodeURIComponent(uid)}`);
        if (response.ok) {
...
export const completeAssessment = async (uid, data) => {
    try {
        ...
        const response = await fetch('/api/v1/skill-practice/complete-assessment', {
            method: 'POST',
...
export const logPracticeSession = async (uid, data) => {
    try {
        const response = await fetch('/api/v1/skill-practice/log-practice', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export const getSkillAnalytics = async (reportId, uid) => {
    try {
        const response = await fetch(`/api/v1/skill-practice/analytics/${reportId}?uid=${encodeURIComponent(uid)}`);
        if (response.ok) {
            return await response.json();
        }
...
export const getPracticeSessions = async (reportId, uid, options = {}) => {
    try {
        ...
        const response = await fetch(`/api/v1/skill-practice/sessions/${reportId}?${params.toString()}`);
        if (response.ok) {
            return await response.json();
        }
